...
import com.example.test.util.AppRTCUtils
import java.util.*

/**
 * AppRTCAudioManager manages all audio related parts of the AppRTC demo.
 */
//음성 입출력을 종합적으로 관리하여 전송하기 위한 클래스
class AppRTCAudioManager private constructor(context: Context) {
    ...
    private inner class WiredHeadsetReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            val state: Int = intent.getIntExtra("state", Companion.STATE_UNPLUGGED)
            val microphone: Int = intent.getIntExtra("microphone", Companion.HAS_NO_MIC)
            val name: String = intent.getStringExtra("name")!!
            Log.d(
                TAG,
                "WiredHeadsetReceiver.onReceive" + AppRTCUtils.getThreadInfo()
                    .toString() + ": " + "a=" + intent.getAction()
                    .toString() + ", s=" + (if (state == Companion.STATE_UNPLUGGED) "unplugged" else "plugged").toString() + ", m=" + (if (microphone == Companion.HAS_MIC) "mic" else "no mic").toString() + ", n=" + name + ", sb="
                        + isInitialStickyBroadcast()
            )
            hasWiredHeadset = state == Companion.STATE_PLUGGED
    ...
    @SuppressLint("WrongConstant")  // TODO(henrika): audioManager.requestAudioFocus() is deprecated.
    fun start(audioManagerEvents: AudioManagerEvents?) {
        ...
        audioFocusChangeListener = object : OnAudioFocusChangeListener {
    ...
    private fun hasWiredHeadset(): Boolean {
        return if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            @SuppressLint("WrongConstant") val devices: Array<AudioDeviceInfo> =
                audioManager.getDevices(AudioManager.GET_DEVICES_ALL)
            for (device in devices) {
                val type: Int = device.getType()
    ...
    fun updateAudioDeviceState() {
        ...
        if (bluetoothManager.getState() === AppRTCBluetoothManager.State.HEADSET_AVAILABLE || bluetoothManager.getState() === AppRTCBluetoothManager.State.HEADSET_UNAVAILABLE || bluetoothManager.getState() === AppRTCBluetoothManager.State.SCO_DISCONNECTING) {
            bluetoothManager.updateDevice()
        }

        // Update the set of available audio devices.
        val newAudioDevices: MutableSet<AudioDevice?> = HashSet()
        if (bluetoothManager.getState() === AppRTCBluetoothManager.State.SCO_CONNECTED || bluetoothManager.getState() === AppRTCBluetoothManager.State.SCO_CONNECTING || bluetoothManager.getState() === AppRTCBluetoothManager.State.HEADSET_AVAILABLE) {
        ...
        val needBluetoothAudioStop =
            ((bluetoothManager.getState() === AppRTCBluetoothManager.State.SCO_CONNECTED
                    ...
                    && (userSelectedAudioDevice != AudioDevice.NONE
                    && userSelectedAudioDevice != AudioDevice.BLUETOOTH))
        if (bluetoothManager.getState() === AppRTCBluetoothManager.State.HEADSET_AVAILABLE || bluetoothManager.getState() === AppRTCBluetoothManager.State.SCO_CONNECTING || bluetoothManager.getState() === AppRTCBluetoothManager.State.SCO_CONNECTED) {
            Log.d(
